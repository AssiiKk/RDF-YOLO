...
class DFM2(nn.Module):
    def __init__(self, dimension=1, Channel1=64, Channel2=64):
        ...
        self.ms_conv  = nn.Sequential(
            nn.Conv2d(self.Channel_all, self.Channel_all, 3, 
                     padding=1, groups=self.Channel_all),  # 标准卷积 
            nn.Conv2d(self.Channel_all, self.Channel_all, 3, 
                     padding=2, dilation=2, groups=self.Channel_all),  # 空洞率2 
            nn.Conv2d(self.Channel_all, self.Channel_all, 3,
                     padding=4, dilation=4, groups=self.Channel_all),  # 空洞率4 
            nn.BatchNorm2d(self.Channel_all*3),
            nn.SiLU(),
            nn.Conv2d(self.Channel_all*3, self.Channel_all, 1)  # 特征压缩 
        )
    ...
    def forward(self, x):
        ...
        # 多尺度特征交互 
        scale1 = fused  # 原始尺度 
        scale2 = self.ms_conv[1](fused)   # 空洞率2 
        scale3 = self.ms_conv[2](fused)   # 空洞率4 
...
class DFM3(nn.Module):
    def __init__(self, dimension=1, Channel1=64, Channel2=64, Channel3=64):
        ...
        self.ms_block  = nn.Sequential(
            nn.Conv2d(self.Channel_all, self.Channel_all, 3, 
                     padding=1, groups=self.Channel_all),  # 局部特征 
            nn.Conv2d(self.Channel_all, self.Channel_all, 3,
                     padding=3, dilation=3, groups=self.Channel_all),  # 中程特征 
            nn.Conv2d(self.Channel_all, self.Channel_all, 3,
                     padding=6, dilation=6, groups=self.Channel_all),  # 全局特征 
            nn.BatchNorm2d(self.Channel_all*3),
            nn.SiLU(inplace=True),
            nn.Conv2d(self.Channel_all*3, self.Channel_all, 1)  # 特征压缩层 
        )
        
        self.w = nn.Parameter(torch.Tensor(self.Channel_all))
        nn.init.constant_(self.w,  1.0)  # 初始化为等权重 
        self.temperature  = nn.Parameter(torch.tensor([0.4]))   # 温度系数调优 
    ...
    def forward(self, x):
        ...
        scale1 = self.ms_block[0](fused)     # 3x3标准卷积 
